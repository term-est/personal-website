...
public:
	...
	Server(asio::ip::tcp::socket socket) noexcept : m_socket{std::move(socket)}
	{}
...
private:
	...
	void receiveHandler(std::shared_ptr<beast::flat_static_buffer<2048>> buffer, std::size_t transfer_size)
	{
		...
		if (m_request.method() == beast::http::verb::get)
		{
			...
			// FIXME: this whole trickery is there for windows
			for (auto &e : req_path)
				if (e == '/')
					e = '\\';
	...
	void asyncResponse(std::shared_ptr<beast::http::response<beast::http::file_body>> response_index)
	{
